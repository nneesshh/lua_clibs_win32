    ...
    if is_repeated then
        ...
        return function (buffer, pos, pend, message, field_dict)
            ...
            while 1 do
                ...
                local next_parse_pos = value:add():_InternalParse(buffer, pos, new_pos)
                if next_parse_pos ~= new_pos then
        ...
        return function (buffer, pos, pend, message, field_dict)
            ...
            local next_parse_pos = value:_InternalParse(buffer, pos, new_pos)
            if next_parse_pos ~= new_pos then
                error('Unexpected end-group tag.')
            end
            return new_pos
